...
def procesar_creditos_api(tmdb_id, num_temporada):
        ...
        if res.status_code == 200:
            ...
            for p in datos.get("cast", []):
                ...
                if not roles:
                    _actor(p, "")
                for r in roles:
                    _actor(p, r.get("character"))
            for p in datos.get("crew", []):
                jobs = p.get("jobs") or []
                if not jobs:
                    _staff(p, p.get("known_for_department") or p.get("department"))
                for j in jobs:
                    _staff(p, j.get("job"))
    ...
    if not creditos_ok:
        ...
        try:
            ...
            if res.status_code == 200:
                datos = res.json()
                for c in datos.get("cast", []):
                    _actor(c)
                for cr in datos.get("crew", []):
                    _staff(cr)
        except Exception:
            pass
    ...
    for r in ratings:
        if r.get("iso_3166_1") in ["US", "MX"]:
            ...
            if rating_code in ["TV-Y", "TV-G", "G"]:
                detalles["publico"] = "Apto para todo público"
            elif rating_code in ["TV-Y7", "TV-Y7-FV", "PG"]:
                detalles["publico"] = "Infantil / Juvenil (7+)"
            elif rating_code in ["TV-PG", "TV-14", "PG-13"]:
                detalles["publico"] = "Adolescentes (14+)"
            elif rating_code in ["TV-MA", "R"]:
                detalles["publico"] = "Animación Adulta (18+)"
            break
    return detalles
...
def detalle_serie(request, serie_id):
    serie = (
        Serie.objects.filter(id_tmdb=serie_id).first()
        or Serie.objects.filter(id=serie_id).first()
    )
    ...
    if serie is None:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if datos.get(
                    "original_language", ""
                ) in IDIOMAS_BLOQUEADOS or 16 not in [
                    g["id"] for g in datos.get("genres", [])
                ]:
                ...
                serie = Serie.objects.create(
                    ...
                    poster_path=(
                        "https://image.tmdb.org/t/p/w500" + poster_raw
                        if poster_raw
                        else None
                    ),
                    backdrop_path=(
                        "https://image.tmdb.org/t/p/original" + backdrop_raw
                        if backdrop_raw
                        else None
                    ),
    ...
    if request.user.is_authenticated:
        progreso_usuario = UserSeriesProgress.objects.filter(
            perfil=request.user.perfil, serie=serie
        ).first()
    ...
    if not tmdb_id:
        try:
            ...
            if res_b.get("results"):
                tmdb_id = res_b["results"][0].get("id")
        except Exception:
            pass
    ...
    try:
        ...
        if response_gen.status_code == 200:
            ...
            serie.total_capitulos = (
                datos_gen.get("number_of_episodes") or serie.total_capitulos
            )
            ...
            if datos_gen.get("poster_path"):
                serie.poster_path = (
                    "https://image.tmdb.org/t/p/w500" + datos_gen["poster_path"]
                )
            if datos_gen.get("backdrop_path"):
                serie.backdrop_path = (
                    "https://image.tmdb.org/t/p/original" + datos_gen["backdrop_path"]
                )
    except Exception:
        pass
    ...
    if not serie.poster_path:
        serie.poster_path = DEFAULT_POSTER
...
def guardar_progreso(request, serie_id):
    ...
    try:
        current_season = int(request.POST.get("current_season", 1))
    except (ValueError, TypeError):
        current_season = 1

    try:
        current_episode = int(request.POST.get("current_episode", 0))
    except (ValueError, TypeError):
        current_episode = 0
...
def index_invitado(request):
                ...
                lista_limpia.append(
                    {
                        ...
                        "fecha_estreno": (
                            item.get("first_air_date", "S/D")[:4]
                            if item.get("first_air_date")
                            else "N/A"
                        ),
                        "generos": (
                            ", ".join(g_nombres[:3]) if g_nombres else "Animación"
                        ),
                        "total_temporadas": 1,
                    }
                )
    ...
    try:
        ...
        opciones_validas = _procesar_lista_api(
            f"{url_populares}&page={pagina_rec}", max_items=20
        )
        ...
        if opciones_validas:
            ...
            detalle = requests.get(
                f"https://api.themoviedb.org/3/tv/{id_tmdb}?api_key={API_KEY_TMDB}&language=es-MX",
                timeout=2.0,
            ).json()
            generos_list = [
                g.get("name", "").strip()
            ...
            Serie.objects.update_or_create(
                ...
                defaults={
                    ...
                    "poster_path": (
                        "https://image.tmdb.org/t/p/w500" + p_rec
                        if p_rec and not p_rec.startswith("http")
                        else p_rec or DEFAULT_POSTER
                    ),
                    "backdrop_path": (
                        "https://image.tmdb.org/t/p/original" + b_rec if b_rec else None
                    ),
    ...
    for s in Serie.objects.all():
        if any(
            p in (s.titulo or "").lower() or p in (s.generos or "").lower()
        ...
        mis_series_filtradas.append(
            {
                ...
                "nota": s.calificacion,
                "generos": (
                    ", ".join([g.strip() for g in s.generos.split(",")][:3])
                    if s.generos
                    else "Animación"
                ),
...
def buscar_series(request):
            ...
            if res.status_code == 200:
                ...
                for item in datos.get("results", []):
                    if (
                        ...
                        and item.get("original_language", "") not in IDIOMAS_BLOQUEADOS
                    ):
    ...
    if request.user.is_authenticated:
        progresos_usuario = {
            ...
            for p in UserSeriesProgress.objects.filter(
                perfil=request.user.perfil
        ...
        lista_render.append(
            {
                ...
                "poster_path": (
                    f"https://image.tmdb.org/t/p/w500{poster_raw}"
                    if poster_raw
                    else DEFAULT_POSTER
                ),
        ...
        if datos.get("results"):
            ...
            if not serie_local:
                ...
                try:
                    ...
                    if (
                        16 not in [g["id"] for g in datos_detalle.get("genres", [])]
                        or datos_detalle.get("original_language", "")
                        in IDIOMAS_BLOQUEADOS
                    ):
                        return redirect("index_invitado")
                    ...
                    serie_local = Serie.objects.create(
                        titulo=datos_detalle.get("original_name")
                        or datos_detalle.get("name"),
                        id_tmdb=id_serie,
                        sinopsis=datos_detalle.get("overview")
                        or "No se encontró sinopsis.",
                        ...
                        generos=(
                            ", ".join([g["name"] for g in lista_generos][:3])
                            if lista_generos
                            else "Animación"
                        ),
            ...
            if request.user.is_authenticated and serie_local:
                ...
                progreso, creado = UserSeriesProgress.objects.get_or_create(
                    ...
                    defaults={
                        "estado": "viendo"
                    },  # O el estado por defecto que prefieras
                )
...
def ver_perfil(request, username):
    ...
    actividades_reales = (
        HistorialActividad.objects.filter(perfil=perfil_usuario)
        .select_related("serie")
        .order_by("-id")
    )  # Añadido un ordenamiento por si acaso para mantener las recientes arriba
...
def guardar_serie_rapido(request):
    try:
        ...
        if not id_tmdb:
            return JsonResponse(
                {"status": "error", "message": "ID de TMDB faltante"}, status=400
            )
...
def votar_actividad(request, actividad_id):
    ...
    voto_existente = VotoActividad.objects.filter(
        perfil=perfil_usuario, actividad=actividad
    ).first()
    ...
    if voto_existente:
        ...
        VotoActividad.objects.create(
            perfil=perfil_usuario, actividad=actividad, tipo=tipo_voto
        )
        estado_voto = "creado"
    ...
    voto_final = VotoActividad.objects.filter(
        perfil=perfil_usuario, actividad=actividad
    ).first()
    ...
    if request.method == "GET":
        try:
            ...
            for c in comentarios:
                usuario_foto = None
                if (
                    hasattr(c.usuario, "perfil")
                    and hasattr(c.usuario.perfil, "foto_perfil")
                    and c.usuario.perfil.foto_perfil
                ):
...
def agregar_comentario(request, actividad_id):
    if request.method == "POST":
        ...
        if not texto:
            return JsonResponse(
                {"status": "error", "message": "El comentario no puede estar vacío"},
                status=400,
            )
        ...
        try:
            ...
            nuevo_comentario = ComentarioActividad.objects.create(
                actividad=actividad, usuario=request.user, texto=texto
            )

            # Buscamos la foto directamente usando la relación del comentario recién guardado
            usuario_foto = None
            if (
                hasattr(nuevo_comentario.usuario, "perfil")
                and hasattr(nuevo_comentario.usuario.perfil, "foto_perfil")
                and nuevo_comentario.usuario.perfil.foto_perfil
            ):
...
def comunidad(request):
    ...
    posts_comunidad = (
        ComunidadPost.objects.all()
        .select_related("usuario", "serie_vinculada", "capitulo_vinculado")
        .prefetch_related("opciones_encuesta")
    )
    # Marcamos estas tarjetas según su tipo real en la base de datos ('foro', 'natural', 'encuesta')
    for post in posts_comunidad:
        post.tipo_tarjeta = (
            post.tipo
        )  # Copia el valor de tu campo 'tipo' (natural, foro, encuesta)
